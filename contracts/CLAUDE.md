...
enterprise-openapi.yaml: OpenAPI 3.1 逻辑协议导航根，定义通用边界、38 个稳定错误码和 81 个 operation，并引用受控 Path Item/schema 分片。
...
fixtures/model-grant-success.json: P2-04 单条 MEMBER 模型授权成功样例。
...
fixtures/quota-policy-success.json: P2-04 成员 quota policy 详情与 nullable limits 成功样例。
...
fixtures/quota-usage-me-success.json: P2-04 员工 ORGANIZATION 生效策略的日/月、RPM 与并发实时计数样例。
...
fixtures/console-bootstrap-success.json: P2-03 产品控制台当前成员、固定角色、权限码与部署标识成功响应样例，不含菜单和部门。
