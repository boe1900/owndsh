...
harness-plugin/ - 独立 pnpm workspace，T01 起承载树外 Harness 企业插件
...
T00 建立上述三个源码与插件工作区；`contracts/` 和 `deploy/` 仍分别由 T02、T21 创建。第三方源码只在产品目录保存无 Git 元数据的锁定快照，同级 `deepseek-harness/` 始终是只读开发依赖。
