...
T00 建立三个上游源码与插件工作区，T01 验证官方插件扩展面，T02 建立跨端协议真源，T03 建立 PostgreSQL、秘密加密、revision CAS 与只追加审计基础；`deploy/` 仍由 T21 创建。第三方源码只在产品目录保存无 Git 元数据的锁定快照，同级 `deepseek-harness/` 始终是只读开发依赖。
