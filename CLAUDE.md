...
plugin/ - 独立 pnpm workspace，面向 DSH Desktop 与普通 Web Host 的官方扩展点构建企业插件并生成 TypeScript/Zod 协议
...
P2-08C 将产品控制台会话收敛为服务端 Sa-Token 与 `__Host-enterprise-admin` Secure/HttpOnly/SameSite=Strict Cookie；管理端以 shadcn authentication 双栏骨架和产品 tokens 原生承载 LOCAL/LDAP 登录，多个 OIDC 仍按身份源独立跳转。浏览器 JavaScript 不读取或保存 Token，管理 API Filter 在 MVC 权限注解前桥接唯一固定 Cookie，新标签直接复用会话；注销和本人改密由服务端撤销会话并清 Cookie，其他标签在下次请求或刷新时返回登录。Desktop 继续使用内存 Bearer Token。
