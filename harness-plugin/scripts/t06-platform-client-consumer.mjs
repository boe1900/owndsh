 ...
 * [POS]: harness-plugin T06 树外包消费者门禁，证明发布产物不借用 workspace 或同级 Harness 源码
...
const platformTgz = resolve(option(
  '--platform-tgz',
  resolve(PROJECT_ROOT, 'artifacts', 'enterprise-agent-dsh-platform-client-0.1.0.tgz'),
))
const contractsTgz = resolve(option(
  '--contracts-tgz',
  resolve(PROJECT_ROOT, 'artifacts', 'enterprise-agent-dsh-contracts-0.1.0.tgz'),
))
...
try {
  ...
  const imported = await run(process.execPath, [
    ...
    '--eval',
    [
      "import * as client from '@enterprise-agent/dsh-platform-client'",
  ...
  const installedRoot = resolve(consumer, 'node_modules', '@enterprise-agent', 'dsh-platform-client')
