...
t11-harness-model-smoke.mjs: T11 真实模型组合验收器，在临时 web profile 通过 rc.2 官方 ctx.llm 与 Agent 恢复层验证动态目录、default、三协议流、xhigh、瞬时 503 重试、终态 quota 零重试、错误矩阵与无本地上游 Key。
...
t17-session-sync-consumer.mjs: T17 树外 consumer，安装产品 tgz 与 npm 官方 rc.2 包并运行真实 SessionStore/JSONL persistence 同步恢复链路。
