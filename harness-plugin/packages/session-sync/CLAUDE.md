...
README.md: T17 Session 客户端边界说明，定义本地优先复制、无正文游标、终态/退避和完整验证后恢复。
...
src/service.ts: 拥有 dirty queue、2 秒防抖、逐 Session 单 worker、断点/退避、远端列表和全量验证后恢复。
src/state-store.ts: 严格读写 `$DSH_HOME/enterprise/session-sync.json`，以 0600 临时文件加 rename 提交无正文游标。
src/types.ts: 定义同步 Config、cursor/status、恢复 DTO 与官方 Service 组合 Context。
...
tests/service.spec.ts: 验证非阻塞 append、flush/readFrom、单 worker、断点、退避终态和坏页零创建。
