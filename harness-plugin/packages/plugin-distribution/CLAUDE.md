...
package.json: 私有 workspace package 清单，固定 rc.7 subprocess/inventory peer 与可打包产品依赖。
tsconfig.json: Node TypeScript 构建边界，从 `src/` 生成 ESM、声明与 sourcemap。
src/cli.ts: 官方 `ctx.subprocess` argv 执行器，显式透传 DSH_HOME 并限制诊断输出。
...
tests/service.spec.ts: 假平台与 Cordis caller proxy 覆盖安装、失败不激活、关闭后重试、重启 active、无 CLI revision 前移、ABSENT、回滚与核心拒绝。
