# @enterprise-agent/dsh-plugin-distribution

Harness Host 的受管插件调和 Service。它只消费 `ctx.enterprisePlatform` 的完整 bootstrap、
官方 rc.7 `ctx.subprocess` 和同进程 `ctx.pluginInventory`，不会扫描或上传个人插件、配置、
源码和本地路径。
...
3. 原子改名为 `<sha256>.tgz`，再以 argv 调用
   `dsh plugin --profile enterprise add --ignore-scripts --save-exact <absolute-tgz>`。
...
移除固定使用 argv `dsh plugin --profile enterprise remove <package-name>`。子进程 seam 会清理
