...
README.md: 员工 Client 边界说明，记录三个官方 slot、共享 store、账号/插件/Session tabs 与同源严格 DTO。
...
src/account-store.ts: 三个 UI slot 与 Settings tabs 共享的外部状态控制器，引用计数管理复合 SSE、账号动作、插件和 Session cursor 状态。
src/account-view.tsx: 账号/插件 settings、sidebar 状态和登录 onboarding 呈现，并编排会话同步 tab。
...
tests/account-store.spec.ts: 共享状态、动作串行、READY bootstrap/插件/Session 加载、恢复删除、SSE 生命周期与错误收敛测试。
