...
package.json: workspace 根清单，固定 Node/pnpm 版本并统一暴露 build/typecheck/test、bundle/contracts/platform-client pack 与 consumer 门禁。
...
packages/: 正式企业插件模块，包含 T06 平台 Service、T07 员工 UI、自包含 bundle 与 T02 contracts；局部地图见 `packages/CLAUDE.md`。
scripts/: contracts/platform-client tarball consumer 与锁定 Harness T01/T07 真实组合验收入口，只操作临时目录/profile。
