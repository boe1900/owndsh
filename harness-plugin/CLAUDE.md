...
package.json: workspace 根清单，固定 Node/pnpm 版本并统一暴露 build/typecheck/test、pack、consumer 与 T11 模型组合门禁。
...
packages/: 正式企业插件模块，包含平台 Service、rc.7 企业模型 adapter、员工 UI、自包含 bundle 与 contracts；局部地图见 `packages/CLAUDE.md`。
scripts/: tarball consumer 与锁定 Harness T01/T07/T11 真实组合验收入口，只操作临时目录/profile。
