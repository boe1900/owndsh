...
package.json: workspace 根清单，固定 Node/pnpm 版本并统一暴露 build、typecheck、test、bundle/contracts pack 与 consumer 门禁。
...
packages/: 正式企业插件模块，包含 T01 技术包、自包含 bundle 与 T02 contracts；局部地图见 `packages/CLAUDE.md`。
scripts/: contracts tarball consumer 与锁定 Harness 真实组合验收入口，只操作临时目录/profile。
