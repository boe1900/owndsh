...
LocalIdentityAdapter.java: 复用 RuoYi BCrypt/失败锁定并强制 bootstrap 首次改密的 LOCAL adapter，以 userId 为稳定 subject。
LocalPasswordChangeRequiredException.java: 旧密码正确但首次改密缺失时的无敏感字段流程信号，不伪装成密码错误。
