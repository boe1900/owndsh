 ...
 * [OUTPUT]: 对外提供当前策略窗口、本人实时用量及管理员筛选分页/聚合查询。
...
public final class QuotaUsageQueryService {
    ...
    public List<WindowUsage> currentWindows(String tenantId, QuotaPolicy policy) {
        Instant now = Instant.now(clock);
        List<WindowUsage> result = new ArrayList<>(2);
    ...
    private PolicyUsage usage(
        ...
        return new PolicyUsage(policy, daily, monthly, rpm, concurrency);
    }
    ...
    private WindowUsage window(
        ...
        QuotaWindowCalculator.WindowBounds bounds = calculator.bounds(now, type);
