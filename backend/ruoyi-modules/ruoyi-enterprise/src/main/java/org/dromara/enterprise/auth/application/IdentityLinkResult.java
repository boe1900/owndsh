/**
 * [INPUT]: 接收 external identity 解析后的平台 userId、首次绑定和部门同步结论。
 ...
 * 身份绑定解析结果。
 */
public record IdentityLinkResult(long userId, boolean linkedNow, boolean userProvisioned, Long departmentId) {
    public IdentityLinkResult {
        if (userId <= 0 || (departmentId != null && departmentId <= 0)) {
            throw new IllegalArgumentException("身份绑定结果 ID 非法");
        }
    }
}
