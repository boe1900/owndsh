 ...
 * [OUTPUT]: 对外提供组映射 seek-page/create/delete，部门/身份源校验、delete CAS 与审计原子提交。
...
public final class IdentityGroupMappingService {
    ...
    public ExternalGroupMapping create(
        ...
        if (!mappings.departmentExists(departmentId)) {
            throw new IllegalArgumentException("部门不存在");
        }
        ExternalGroupMapping mapping = new ExternalGroupMapping(
            positiveId(), context.tenantId(), sourceId, externalGroup, departmentId, 0
        );
        try {
