...
GatewayChatRequest.java: 经严格校验的请求局部值，只在上游发送前替换受管模型并强制 usage 流。
GatewayChatRequestParser.java: OpenAI-compatible 顶层白名单、纯文本 message/tool 校验与可见字节估算入口。
...
ModelGatewayService.java: 配额预留、SENT、短事务审计、SSE 转发、续租与 SETTLED/CHARGED_MAX 的生命周期编排。
