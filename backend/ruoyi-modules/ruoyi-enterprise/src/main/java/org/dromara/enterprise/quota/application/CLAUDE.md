...
QuotaPolicySpec.java: nullable 独立限额与 DEFAULT/DEPT/USER 主体不变量 command。
...
EffectiveQuotaResolver.java: DEFAULT、当前 DEPT 与 USER ACTIVE 策略按 ID 排序叠加解析器。
...
QuotaUsageQueryService.java: 当前窗口/RPM/并发、本人有效策略和管理员 ledger 聚合查询。
