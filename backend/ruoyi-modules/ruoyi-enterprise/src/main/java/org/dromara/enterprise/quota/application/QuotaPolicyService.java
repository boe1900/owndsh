...
public final class QuotaPolicyService {
    ...
    public QuotaPolicy create(QuotaMutationContext context, QuotaPolicySpec spec) {
        ...
        Long id = transactions.execute(status -> {
            ...
            QuotaPolicy policy = policy(policyId, context.tenantId(), spec, 0);
    ...
    public QuotaPolicy update(
        ...
        transactions.executeWithoutResult(status -> {
            ...
            QuotaPolicy next = policy(id, context.tenantId(), spec, expectedRevision + 1);
    ...
    private void requireSubject(QuotaPolicySpec spec) {
    ...
    private static QuotaPolicy policy(long id, String tenantId, QuotaPolicySpec spec, long revision) {
        return new QuotaPolicy(
            id, tenantId, spec.name(), spec.subjectType(), spec.subjectId(), null,
            spec.dailyTokenLimit(), spec.monthlyTokenLimit(), spec.rpm(), spec.concurrency(),
    ...
    private static QuotaPolicy withStatus(QuotaPolicy current, QuotaStatus status) {
        return new QuotaPolicy(
            current.id(), current.tenantId(), current.name(), current.subjectType(), current.subjectId(),
            current.subjectName(), current.dailyTokenLimit(), current.monthlyTokenLimit(), current.rpm(),
            current.concurrency(), status, current.revision() + 1
        );
    }
