...
pom.xml: Maven 聚合根，集中声明 Java 21、Spring Boot 4.1、各 RuoYi 子模块与 T02 JSON Schema validator 测试版本。
ruoyi-admin/: Spring Boot 应用装配层，承载 T01 Sa-Token 与 T02 协议验收，后续接入 ruoyi-enterprise 运行依赖；局部地图见 `ruoyi-admin/CLAUDE.md`。
...
ruoyi-modules/: 业务模块聚合层，T03 在此新增边界独立的 ruoyi-enterprise 模块。
