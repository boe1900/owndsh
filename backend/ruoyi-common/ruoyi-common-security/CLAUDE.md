...
src/main/java/org/dromara/common/security/config/SecurityConfig.java: 全局 Sa-Token Servlet/filter 与路由鉴权入口；Actuator Basic Auth 默认开启但允许最小 health-only 部署关闭，企业 API 由领域层读取 Token session 裁决固定 client。
src/main/java/org/dromara/common/security/config/properties/SecurityProperties.java: 登录白名单与 Actuator filter 条件装配前缀的强类型配置边界。
