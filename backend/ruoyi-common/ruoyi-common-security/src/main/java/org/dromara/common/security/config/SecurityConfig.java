 ...
 * [OUTPUT]: 提供异步分发可用的 Sa-Token filter、可配置 Actuator Basic Auth，以及把企业 API 鉴权完整下沉到领域 context 的统一拦截器。
 * [POS]: ruoyi-common-security 的全局入口；非企业路由保留登录/clientid 交叉校验，企业路由避免在领域撤销语义前截断请求。
