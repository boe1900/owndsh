 ...
 * [OUTPUT]: 提供产品页面静态元数据、路径判断和多角色页面并集。
 * [POS]: app 的唯一导航与前端页面可见性真源，Server ent:* 权限仍独立裁决 API。
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */

import { Activity, Boxes, Puzzle, Settings, ShieldCheck, Users, type LucideIcon } from 'lucide-react';
import type { AuthBuiltInRole } from '@/api/generated/types.gen';

type ProductRouteDefinition = {
  to: '/' | '/access' | '/plugins' | '/members' | '/activity' | '/settings';
  label: string;
  icon: LucideIcon;
...
export const PRODUCT_ROUTES = [
  ...
  { to: '/settings', label: '设置', icon: Settings, allowedRoles: ['enterprise_admin'] }
] as const satisfies readonly ProductRouteDefinition[];

export type ProductRoute = (typeof PRODUCT_ROUTES)[number]['to'];

export function isProductRoute(pathname: string): pathname is ProductRoute {
  return PRODUCT_ROUTES.some((route) => route.to === pathname);
}
