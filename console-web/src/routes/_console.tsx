/**
 * [INPUT]: 依赖标签页会话、console bootstrap、静态角色路由与产品 ConsoleShell。
...
import { isProductRoute, productRoutesFor } from '@/app/product-routes';
import { AuthRequiredError, getToken, loadConsoleBootstrap } from '@/auth/session';

export const Route = createFileRoute('/_console')({
  beforeLoad: async ({ location }) => {
    if (!getToken()) throw redirect({ to: '/login', search: { redirect: location.href }, replace: true });
    let bootstrap;
    try {
    ...
    if (!isProductRoute(location.pathname) || !routes.some((route) => route.to === location.pathname)) {
