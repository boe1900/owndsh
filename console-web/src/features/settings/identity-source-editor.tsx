 ...
 * [OUTPUT]: 提供 OIDC/LDAP 创建、OIDC/LDAP/LOCAL 编辑及不回显 secret 的请求体构造。
 * [POS]: features/settings 的身份源写入边界，只短暂持有用户本次输入的 secret，不读取或伪造服务端秘密。
      ...
      <form className="grid gap-4 p-5" onSubmit={(event) => { event.preventDefault(); submit(); }}>
        ...
        {value.type === 'LDAP' ? (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <TextField label="组属性" value={value.groupAttribute} onChange={(next) => set('groupAttribute', next)} />
