/**
 * [INPUT]: 依赖 TanStack Form、Zod、ProductDialog、生成的 Provider/模型 DTO 与十进制容量转换。
 * [OUTPUT]: 提供 Provider 编辑器、受管模型编辑器和模型删除确认对话框。
 * [POS]: features/models 的纯表单层，忠实收集 Harness 模型声明，不持有权限、查询缓存或 Server mutation。
...
function providerDefaults(current?: Provider): ProviderFormValue {
  return {
...
export function ProviderEditorDialog({
  ...
  error?: string;
  onClose: () => void;
  onSave: (value: ProviderCreateRequestWritable | ProviderUpdateRequestWritable) => void;
  saving: boolean;
}) {
  const form = useForm({
    defaultValues: providerDefaults(current),
    onSubmit: ({ value }) => {
      const common = {
