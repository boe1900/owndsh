...
compose.yml: 默认拉取 GHCR `next` Server/Console，以带默认值的环境变量和可覆盖 `admin/owndsh` 初始化空库；签名默认关闭且不提供私钥，源码与离线 release 共用拓扑，数据组件 digest 固定且仅 Console 发布 HTTP 8080；持久卷只保存 PostgreSQL、Redis 和插件制品，storage-init 仅初始化插件卷权限。
