...
本目录交付详细设计 T21 的单企业、单 Server、Linux `amd64` 部署。生产入口只有 Console 容器的 `443`；Server、PostgreSQL 和 Redis 没有宿主端口。Console 与管理 API 同域，Nginx 覆盖客户端提供的 `Forwarded`/`X-Forwarded-*`，Server 只信任这一跳。
...
恢复会短暂停止 Console、Server 和 Redis，并覆盖目标安装中的数据库、Redis、artifact 与关键 key。恢复脚本先校验 Redis RDB，再由隔离的 Redis 进程加载 RDB 并生成 AOF，避免开启 AOF 的常规进程忽略独立 RDB；TLS 由目标域名单独维护：
