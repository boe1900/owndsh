...
build-release.sh: 以锁定 digest 构建 Server/console-web Gateway，允许显式 registry 前缀或经 RepoDigest 反查验证的本地缓存，并从 Harness 机器锁生成 manifest 后打包 PostgreSQL 基线、企业 bundle、许可证、运维文档和校验和。
...
upgrade.sh: 先备份，再加载新 release、同步待分发 Harness bundle 并启动新 Server/Gateway；保留安装专属 overlay 和旧应用引用供回滚。
rollback.sh: 恢复上一 release 的 Compose 指针、Server/Gateway 镜像与待分发 Harness bundle，使同一新版可再次前滚，并显式证明数据库卷、Redis 卷、artifact 卷与 key 指纹未变。
