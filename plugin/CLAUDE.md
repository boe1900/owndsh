...
README.md: OwnDsh 标准 Harness 插件 workspace 说明，记录 RC1 官方插件管理页同步、受控企业目录、初装只填 Server 与树外发布边界。
...
pnpm-lock.yaml: workspace 锁定依赖图，固定编译、测试、React Client、OpenAPI 生成与 npm 官方 Harness `0.1.7-rc.1` 开发依赖，使发布构建不依赖同级 checkout。
...
packages/: 正式企业插件模块，包含平台 Service、兼容 Harness 模型配置桥、官方插件管理页同步、门禁 UI、自包含 bundle 与 contracts；局部地图见 `packages/CLAUDE.md`。
