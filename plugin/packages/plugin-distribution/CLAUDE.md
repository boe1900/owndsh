...
src/service.ts: Cordis shadow-compatible 的串行 revision 调和、缺失信任根 fail-closed、核心保护、重启确认、库存替换和整包卸载所有者。
src/state-store.ts: `managed-plugins.json` 严格解析与私有权限原子替换边界。
src/types.ts: 平台、subprocess、Loader inventory 窄 port 及可选安装信任根/已验证 Harness commit/受管状态契约。
src/verification.ts: 流式下载、大小/hash、RFC 8785 受限声明、Ed25519 与已知 commit fail-closed compatibility 校验。
tests/service.spec.ts: 假平台与 Cordis caller proxy 覆盖 Web/Desktop 命令口、安装、无信任根拒绝、重启 active、ABSENT、回滚、核心保护和整包卸载顺序。
