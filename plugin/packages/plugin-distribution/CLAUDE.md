...
README.md: 官方 pluginManager 安装/更新/卸载入口、固定版本身份、私有源认证与破坏性迁移说明。
package.json: 私有 workspace package 清单，声明兼容 Harness pluginManager/inventory peer 与可打包产品依赖。
...
src/service.ts: 企业可选目录和显式安装/卸载的串行所有者；点击时重查授权，绑定版本 ID，委托官方 pluginManager 安装/移除并核对结果，重启后将 ABSENT 记录收敛为未安装并上报库存。
src/state-store.ts: plugin-installations.json 的严格解析与私有权限原子替换，不读取旧制品状态文件。
src/types.ts: 官方平台/pluginManager/inventory 窄 port、必填安装配置和版本 ID 状态，配置只包含运行参数。
