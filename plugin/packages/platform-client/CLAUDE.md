...
package.json: 私有 workspace package 清单，声明 caret-compatible Cordis/settings/Schemastery peers、T02 contracts 与 strict Zod 运行依赖。
...
src/platform-service.ts: 注册 `ctx.enterprisePlatform`，从官方 settings 解析 HTTP(S) Server origin，承载内存认证、控制面限时/SSE 无总时限、bootstrap 轮询、状态订阅，并在 dispose 时等待 installation 与网络任务停稳。
src/types.ts: 复用生成契约严格校验 Bootstrap、模型、配额与受管插件，定义含 `UNCONFIGURED` 和可空平台地址的脱敏状态 DTO。
...
tests/platform-service.spec.ts: 真实 socket 下验证 HTTP(S) origin、官方 settings 地址持久化/切换、登录/bootstrap、状态订阅隔离、控制面超时、轮询退避与停稳。
