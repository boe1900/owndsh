...
src/pkce.ts: PKCE S256 生成、仅绑定 `127.0.0.1` 的 callback、state/取消/超时生命周期；启动前或监听期间取消也由 callback.result 返回稳定失败，不泄漏未处理拒绝。
...
tests/pkce.spec.ts: S256、精确 callback、state、启动前/监听期间取消和超时的 Vitest 验收。
tests/platform-service.spec.ts: 真实 socket 下验证安装配置 bootstrap、退出后才可修改 Server、通用 settings 不可绕过、保存/认证互斥与旧账号不复活，以及 GrantRecord 恢复、按需轮换、超时、闲置零请求和退出竞态。
