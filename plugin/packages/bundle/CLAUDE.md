...
src/mcp-oauth.ts: 从 bootstrap 身份与公共目标摘要隔离 MCP grant；access token 仅内存，refresh/API Key 与动态 public clientId 走官方原子记录，复用 PKCE loopback/DCR，授权/发现/注册/交换/刷新统一接受管理员指定的 HTTP(S)，保留 userinfo/片段/重定向拒绝和 issuer/resource 绑定，区分失效授权与瞬时刷新失败，阻断取消/销毁后的迟到写入。
