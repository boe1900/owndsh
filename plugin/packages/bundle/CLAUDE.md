...
package.json: npm 发布清单与 `dsh.bundle`/`dsh.client` 双入口，官方 Harness 依赖锁定 `0.1.6-alpha.2`，MCP OAuth/transport 使用官方 `@modelcontextprotocol/client` v2。
...
cordis.patch.yml: 官方 profile layer，覆盖企业 default、停用个人 provider/模型设置与官方插件管理页，并插入企业 Host/Client；MCP resources 复用 alpha.2 base 已有 row。
