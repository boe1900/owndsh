/**
 * [INPUT]: 依赖 官方 remote/locale/layout/sidebar 服务与本地页面
 * [OUTPUT]: 对外提供 apply、PANEL_ID、官方插件管理注册
 * [POS]: OwnDsh 对官方 ui-plugin-manager 的最小 fork 入口
...
import type { PluginInstallLogChunk, PluginInstallProgress } from '@deepseek-ai/dsh-api-remotes/client'
...
import { PluginsPanelIcon } from './PluginsPanelIcon.js'
import { configLedgerSource } from './config-ledger.js'
import { PluginManagerController } from './manager-store.js'
import { en, zh, type PluginManagerLocaleKey } from './locales.js'
import type {} from './slot-contract.js'

export type { PluginManagerPageProps } from './PluginManagerPage.js'
export type { ConfigLedger, OfficialItem } from './config-ledger.js'
export type { PluginManagerFace } from './manager-store.js'
export type { PluginManagerLocaleKey } from './locales.js'
export type {
 ...
 * @param openMarket - opens the OwnDsh-controlled plugin market.
 */
export function apply(ctx: ClientContext, openMarket: () => void): void {
  ...
  ctx.effect(() => {
    ...
    const disposers = [
      ...
      ctx.remote.$on('plugin-manager/install-log', (chunk: PluginInstallLogChunk) => { controller.appendLog(chunk) }),
      ctx.remote.$on('plugin-manager/install-state', (progress: PluginInstallProgress) => { controller.installProgress(progress) }),
      (ctx as unknown as { on(event: 'connection/reset', listener: () => void): () => void }).on('connection/reset', refresh),
    ]
  ...
  ctx.slots.inject('main', () => ctx.slots.register({
    ...
    key: PANEL_ID,
    locale: NS,
    inject: () => controller.inject(configLedger, text => ctx.locale.resolveText(text), openMarket),
    children: {
