/**
 * [INPUT]: 依赖 官方 pluginManager Remote 与 configForms
 * [OUTPUT]: 对外提供 PluginManagerController、PluginManagerFace
 * [POS]: 官方插件安装生命周期与列表状态控制器
...
import type { ConfigLedger } from './config-ledger.js'
import { shortName } from './presentation.js'
...
export interface PluginManagerFace {
  ...
  /** Read the Host again. */
  refresh: () => void
  /** Open the OwnDsh-controlled plugin market from the page header. */
  openMarket: () => void
  openInstall: () => void
...
export class PluginManagerController {
  ...
  inject(
    configLedger: HostObservable<ConfigLedger>,
    resolveText: PluginManagerFace['resolveText'],
    openMarket: PluginManagerFace['openMarket'],
  ): PluginManagerFace {
    return {
      resolveText,
      openMarket,
  ...
  private async read(): Promise<void> {
    try {
      do {
        ...
        this.patch({
          status: 'ready',
          packages: sortPackages(bundles.value.map((bundle: BundleInfo) => packageView(bundle, plugins.value))),
        })
