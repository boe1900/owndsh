...
src/account-view.tsx: 对齐宿主 tokens 的账号/插件 settings、两个入口共享的确认登出动作与内嵌 OwnDsh 品牌鲸图的全屏访问门禁，以单行 Server 编辑、状态线和封闭键盘焦点覆盖未配置/未登录/失效，并提供整包卸载。
...
tests/account-view.spec.ts: 锁定连接/受管插件状态、门禁放行条件、确认登出以及重启/失败员工语义。
