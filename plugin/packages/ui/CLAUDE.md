...
src/account-store.ts: 官方 slot 共享的状态控制器，串行处理 Server、账号和插件安装/卸载；宿主事件/revision 变化只读取本地事实，登录中临时查询有截止时间，不产生安装动作，插件筛选状态由设置页局部拥有。
src/account-footer.tsx: 官方 sidebar footer slot 的 OwnDsh 账户行，使用品牌图标、显示名和灰色确认退出按钮；插件管理归设置页，不劫持 Settings 私有状态。
src/account-view.tsx: 复用宿主 Button/tokens，以账号与登录状态合并摘要、带图标的地址/设备/版本分组信息和底部显式刷新配置/退出/卸载承载设置；分组底色与宿主超椭圆边框配套，长值单行省略且不显示连接时间；保留插件 tab、官方 close 门禁联动和窄屏导航适配。
src/confirm-action.tsx: 复用 Harness 共享 Modal/Button 的页面确认，封闭焦点并隔离外层 Escape，只有明确确认才调用业务动作，供账号与卸载入口共用。
src/plugin-market.tsx: OwnDsh 设置内的插件管理视图，分离企业目录和本机库存，以搜索、卡片、详情和已安装筛选承载显式安装/版本切换/确认卸载；详情弹窗管理焦点并隔离外层 Settings 的 Escape。
...
tests/account-store.spec.ts: 共享状态、Server/账号/卸载动作串行、连接/revision 去重、Session 零请求、有界登录查询、过期响应隔离与错误收敛测试。
