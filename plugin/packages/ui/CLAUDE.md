...
src/local-api.ts: 固定同源路径的严格 Server/账号/卸载/插件/MCP DTO 与显式刷新解码，承载 OAuth/API Key 本地动作、严格 OAuth flow 进度与连接意愿/名称简介目录/发现数量/有效呈现事实，删除 SHA/hash/marker 并拒绝 Token、正文和执行细节。
