 ...
 * [OUTPUT]: 验证订阅生命周期、账号动作、连接/revision 去重加载、V1 Session 停用与错误投影
...
describe('EnterpriseAccountStore', () => {
  it('shares SSE state, serializes actions, loads READY account facts, and disposes', async () => {
    ...
    publish({ ...current, state: 'REFRESHING', revision: 7 })
    publish({ ...current, state: 'READY', revision: 7 })
    expect(api.bootstrap).toHaveBeenCalledOnce()
    expect(api.plugins).toHaveBeenCalledOnce()
    publish({ ...current, state: 'READY', revision: 8 })
