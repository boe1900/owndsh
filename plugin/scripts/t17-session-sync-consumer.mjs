...
const sessionSyncTgz = resolve(option(
  '--session-sync-tgz', resolve(PROJECT_ROOT, 'artifacts', 'owndsh-session-sync-0.1.0.tgz'),
))
...
try {
  ...
  await writeFile(resolve(consumer, 'package.json'), JSON.stringify({
    ...
    dependencies: {
      ...
      '@owndsh/session-sync': `file:${sessionSyncTgz}`,
    },
  }, null, 2))
  ...
  const installedRoot = resolve(consumer, 'node_modules', '@owndsh', 'session-sync')
