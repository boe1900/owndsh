...
t01-harness-smoke.mjs: T01/T06/T14 真实组合验收器，将零业务配置 bundle 安装到锁定临时 profile，验证 `UNCONFIGURED`、Server API/settings 持久化、Client bundle、本地状态/插件 API/SSE 与上游清洁度。
...
t17-harness-session-smoke.mjs: T17 锁定 rc.2 组合门禁，以 gated 假平台和真实 JSONL backend 验证非阻塞 append、确认游标、远端列表与新 ID 恢复。
t17-session-sync-consumer.mjs: T17 树外 consumer，安装声明兼容 peer 的产品 tgz 与 npm 官方 rc.2 包并运行真实 SessionStore/JSONL persistence 同步恢复链路。
