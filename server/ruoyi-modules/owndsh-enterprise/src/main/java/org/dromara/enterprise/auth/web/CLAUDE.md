...
AdminSessionCookie.java: 固定 `__Host-enterprise-admin` 的 Secure/HttpOnly/SameSite=Strict/host-only 属性与 4 KiB 值边界，并集中拒绝显式跨源的非安全方法请求。
...
PlatformAuthController.java: authorize/sources/password/OIDC/Desktop token/浏览器 Cookie/logout 门面；管理端专用交换不返回 Token，页面 JSON 以 CHANGE_PASSWORD 或精确回调驱动两阶段流程。
