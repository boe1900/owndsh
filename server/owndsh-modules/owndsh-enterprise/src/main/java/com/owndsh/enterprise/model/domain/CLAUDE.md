...
ProviderApiProtocol.java: 对齐 Harness 自定义路由的 openai-completions、openai-responses、anthropic-messages 三种 wire protocol 标识；DeepSeek 官方 provider 在 RC1 固定使用 anthropic-messages。
...
ModelProvider.java: provider 聚合根，持有稳定 Harness providerKey、来源、API 协议、endpoint、timeouts 和 AES-GCM 密文；在领域层固定官方 DeepSeek 的 Messages 协议但禁止密文直接进入 Web。
