...
java/com/owndsh/test/EnterpriseSafetyDefaultsTest.java: 结构化读取唯一 application.yml，验证 Flyway/JDBC、请求上限、graceful drain、CORS、插件签名与环境入口；以真实全局过滤器验证三协议长 JSON/工具参数/HTML 字节保留及普通接口继续清洗，加载真实 Logback 配置验证仅 stdout 输出、级别与异常堆栈保留。
