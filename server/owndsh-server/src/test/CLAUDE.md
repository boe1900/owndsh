...
java/com/owndsh/test/EnterpriseSafetyDefaultsTest.java: 结构化读取唯一 application.yml，验证请求上限、graceful drain、同源 CORS，默认关闭插件签名以及 PostgreSQL/Redis/JWT/master/signing key 环境入口；加载真实 Logback 配置验证仅 stdout 输出、级别与异常堆栈保留。
