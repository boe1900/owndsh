/**
 * [INPUT]: 依赖宿主认证策略、客户端配置、Sa-Token 会话、社交登录与消息服务。
 * [OUTPUT]: 对外提供兼容宿主客户端的登录、注册、绑定和注销 HTTP 接口。
 * [POS]: owndsh-server 的宿主认证兼容入口；OwnDsh 企业登录由 owndsh-enterprise 独立状态机承载。
        ...
        scheduledExecutorService.schedule(() -> {
            messageService.publishMessage(
                ...
                PushPayloadDTO.of(
                    ...
                    DateUtils.getTodayHour(new Date()) + "好，欢迎登录 OwnDsh",
                    null
                )
            );
        }, 5, TimeUnit.SECONDS);
