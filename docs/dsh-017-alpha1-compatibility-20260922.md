...
- 官方 DeepSeek adapter 改为 Messages-only 不会造成 OwnDsh 插件装载断点：插件使用 `dsh-llm-pi-ai`，并通过 patch 禁用 `llm-deepseek`。但这条结论不适用于企业模型的 provider 语义；`DEEPSEEK_OFFICIAL` 必须跟随官方 API，直接使用 `anthropic-messages` 与 `/anthropic` 根地址，不能默认 OpenAI Completions。
