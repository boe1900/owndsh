...
| `ent_usage_ledger` | `id bigint,tenant_id,reservation_id,user_id,model_id,request_id,input_tokens,output_tokens,cache_tokens,total_tokens,result,upstream_request_id,created_at` | 唯一 `reservation_id`；索引 `(tenant_id,created_at,id)`、`(user_id,created_at)`、`(model_id,created_at)`、`request_id` |
