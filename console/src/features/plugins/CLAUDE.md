...
plugin-editors.tsx: 首次登记与新增版本共用表单，新增时沿用资料并折叠、锁定包名，npm 默认目标随版本生成，Git 必须更换 commit；发布明确选择沿用某旧版 ACTIVE/INSTALLED 范围或仅发布，携带已查看的包 revision；分类多选/自定义、范围编辑与退休沿用现有组件。
